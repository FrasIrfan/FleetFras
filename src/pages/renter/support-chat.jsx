...
export default function RenterSupportChatPage() {
  ...
  useEffect(() => {
    ...
    const fetchChat = async () => {
      const q = query(collection(db, 'chats'), where('userId', '==', currentUser.uid), where('userRole', '==', 'renter'));
  ...
  const sendMessage = async () => {
    ...
    const newMsg = { sender: 'renter', text: input, timestamp: Date.now() };
    ...
    <div className="ff-page flex flex-col items-center">
      <div className="ff-shell flex min-h-[720px] max-w-3xl flex-col p-0">
        <div className="border-b px-6 py-6 sm:px-8" style={{ borderColor: 'var(--ff-border)' }}>
          ...
          <p className="mt-2 text-sm text-slate-500">Ask admins about listings, approvals, and renter support.</p>
        </div>
        <div className="flex flex-1 flex-col bg-slate-50/70 p-4 sm:p-6">
          ...
          {messages.length === 0 ? (
            ...
            messages.map((msg, idx) => (
              <div
                key={idx}
                className={`mb-3 flex ${msg.sender === 'renter' ? 'justify-end' : 'justify-start'}`}
              >
                <div className="max-w-[70%]">
                  <div className={`mb-1 text-xs ${msg.sender === 'renter' ? 'text-right text-purple-700' : 'text-left text-slate-500'}`}>{msg.sender === 'renter' ? 'You:' : 'Admin:'}</div>
                  <span
